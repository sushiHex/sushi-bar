...
**git branch** (with session name when it differs from the dir) │ **model** │ **context bar** (quiet gray gauge) │ **5h & 7d usage-limit trackers** (% used + time-to-reset, green→yellow→red) │ **working dir**. Also keeps the terminal/tab title set.
