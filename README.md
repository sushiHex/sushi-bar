...
### 🖼️ snip — draw-box screenshot into Claude

Run `/snip` → drag a box anywhere on screen → Claude reads the shot in the same turn. Pass a question to act on it: `/snip what's wrong with this error?`.

- **Always a rectangular draw-box** (its own overlay, not the OS Snip's remembered mode) — multi-monitor and DPI-correct.
- **Auto-read:** the `/snip` command captures *and* loads the image for Claude in one step.
